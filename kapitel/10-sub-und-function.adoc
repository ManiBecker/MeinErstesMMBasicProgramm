...
Eine indexterm:[SUB-Befehlsfamilie,SUB]indexterm:[SUB]`SUB` fasst mehrere Anweisungen zu einem indexterm:[Unterprogramme]Unterprogramm zusammen.
...
Die Anweisungen zwischen `SUB` und indexterm:[SUB-Befehlsfamilie,END SUB]indexterm:[END SUB]`END SUB` werden erst ausgeführt, wenn die `SUB` aufgerufen wird.
...
Dafür gibt es indexterm:[FUNCTION-Befehlsfamilie,FUNCTION]indexterm:[FUNCTION]`FUNCTION`.
...
Die Funktion wird mit indexterm:[FUNCTION-Befehlsfamilie,END FUNCTION]indexterm:[END FUNCTION]`END FUNCTION` abgeschlossen. Sie erhält einen Wert als Parameter und liefert das Quadrat dieses Wertes zurück.
