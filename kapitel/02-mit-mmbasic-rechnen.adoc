...
image::bilder/02_mit_mmbasic_rechnen.jpg[align=center]
...
Im letzten Kapitel haben wir mit indexterm:[PRINT]`PRINT` Text auf dem Bildschirm ausgegeben.
...
=== Die vier indexterm:[Grundrechenarten]Grundrechenarten
...
=== indexterm:[Klammern, Rechenausdrücke]Klammern verwenden
...
=== indexterm:[Potenzierung]Potenzen berechnen
...
Die Funktion indexterm:[Mathematische Funktionen, SQR()]indexterm:[SQR()]`SQR()` berechnet die indexterm:[Quadratwurzel]Quadratwurzel einer Zahl.
...
Die Funktion indexterm:[Mathematische Funktionen, ABS()]indexterm:[ABS()]`ABS()` liefert den indexterm:[Betrag]Betrag einer Zahl.
...
* {github-programme}/02_rechenarten.bas[02_rechenarten.bas]
endif::[]
