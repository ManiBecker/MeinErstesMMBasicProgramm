...
Mit einer indexterm:[Schleifen]Schleife geht es deutlich einfacher.

=== Unsere erste Schleife

[source,basic]
----
indexterm:[FOR...NEXT-Befehlsfamilie,FOR]indexterm:[FOR]FOR I=1 TO 10
  PRINT "Hallo Welt!"
indexterm:[FOR...NEXT-Befehlsfamilie,NEXT]indexterm:[NEXT]NEXT I
----
...
MMBasic erzeugt eine indexterm:[Zählvariable]Zählvariable mit dem Namen `I`.
...
Dazu wird indexterm:[FOR...NEXT-Befehlsfamilie,STEP]indexterm:[STEP]`STEP` verwendet.
...
Im folgenden Beispiel kommt der indexterm:[MOD]Operator `MOD` zum Einsatz. Er berechnet den *Rest einer Division*.
