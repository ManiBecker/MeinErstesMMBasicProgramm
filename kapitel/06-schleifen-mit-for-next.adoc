...
image::bilder/06_schleifen_mit_for_next.jpg[align=center]
...
indexterm:[Schleifenvariable]

[IMPORTANT]
====
Die Schleifenvariable wird von `FOR ... NEXT` automatisch verwaltet. Verändere
ihren Wert deshalb innerhalb der Schleife möglichst nicht selbst.
...
Hier wird `I` innerhalb der Schleife verändert und anschließend durch `NEXT`
noch einmal erhöht. Dadurch ist der Ablauf schwieriger nachzuvollziehen.

Wenn eine Schleife unter bestimmten Bedingungen vorzeitig beendet werden soll,
verwende stattdessen `EXIT FOR`.
====
...
Eine `FOR ... NEXT`-Schleife kann auch eine weitere Schleife enthalten.
Man spricht dann von indexterm:[verschachtelte Schleifen] *verschachtelten
Schleifen*.
...
Bei jedem Durchlauf der äußeren Schleife wird die innere Schleife vollständig
durchlaufen. In unserem Beispiel läuft `ZEILE` dreimal von 1 bis 3. Für jede
Zeile läuft `SPALTE` fünfmal von 1 bis 5.

=== Eine Schleife vorzeitig verlassen

Normalerweise wird eine `FOR ... NEXT`-Schleife so lange ausgeführt, bis ihr
Endwert erreicht ist. Manchmal soll die Schleife jedoch schon vorher beendet
werden.

Dafür gibt es indexterm:[EXIT FOR] `EXIT FOR`.
...
Sobald `I` den Wert 5 erreicht, wird `EXIT FOR` ausgeführt. Das Programm
verlässt die Schleife und setzt die Ausführung hinter `NEXT I` fort.
...
* {github-programme}/06_schleifen.bas[06_schleifen.bas]
endif::[]
